...
5. [Attendance & Shift Management](#attendance--shift-management)
6. [Leave Management](#leave-management)
7. [Payroll & Loans](#payroll--loans)
8. [Reports & Exports](#reports--exports)
9. [System Configuration](#system-configuration)
10. [Subscription & Access](#subscription--access)
...
1. **View Current Attendance**
   - Go to **"Attendance"** → **"Daily Attendance"** to see today's attendance list
...
2. **Absences**
   ...
   - Managers can regularize or adjust records using the admin screens if needed
   - Absence reasons can be recorded

3. **Overtime Tracking (if enabled)**
   - Overtime is calculated based on configured working hours and attendance
   - Overtime amounts are considered during payroll processing where applicable

### **Shift Management**

1. **Creating Shifts**
   - Navigate to **"Shift Management"** → **"Create Shift"**
   - Define shift name, start/end time, and other rules
   - Save the shift so it can be assigned to employees

2. **Assigning Shifts**
   - Go to **"Shift Management"** → **"Assign Shift"**
   - Select employee(s) and choose the appropriate shift
   - Apply assignments for specific dates or periods

3. **Reallocating Shifts**
   - Use **"Shift Management"** → **"Shift Reallocation"**
   - Adjust shifts when employees change teams, departments, or work patterns
   - Review changes before saving to avoid conflicts
...
3. **Calculate Salaries**
   ...
   - Adds allowances and bonuses (from **Allowance** setup)
   - Deducts leaves and other deductions (from **Deduction** setup)
   - Optionally includes overtime payments where configured
...
3. **Export and Distribution**
   - Export payroll in **Excel** or **PDF** format
   - Generate individual salary slips
   - Distribute to employees

### **Loan & Advance Management**

1. **Creating Loans/Advances**
   - Navigate to **"Loan"** → **"Add Loan/Advance"**
   - Enter employee, amount, repayment terms, and other required details
   - Save the record so it is linked with payroll

2. **Managing Loan Records**
   - Go to **"Loan"** → **"Loan/Advance"**
   - View all active and closed loans/advances
   - Edit or close records as repayments complete

3. **Impact on Payroll**
   - Loan/advance deductions appear automatically in **Monthly Payroll**
   - Review these amounts in the payroll detail before finalizing
...
2. **Monthly & Muster Reports**
   - Go to **"Reports"** → **"Monthly Report"** or **"Monthly Muster Preview"**
   - Select month and year
   - View monthly attendance trends and muster details
   - Analyze attendance patterns for the whole organization
...
4. **Salary & Payroll Reports**
   - Navigate to **"Reports"** → **"Monthly Salary Report"**
   - View salary distribution and payroll cost summaries
   - Use filters (branch, department, designation, etc.) for deeper analysis

5. **Date Range & Detailed Reports**
   - Go to **"Reports"** → **"Date Range Report"**
   - Select a custom date range
   - Generate consolidated reports for specific periods
   - Compare data across different timeframes

6. **Geo-Location & Detailed Daily Reports**
   - Use **"Geolocation Report"** to view attendance based on punch locations
   - Use **"Detailed Daily Report"** for more granular, per-day information

### **Report Features**

- **Filtering Options**: Filter by department, branch, designation
- **Export Options**: Excel and PDF export available (where supported)
- **Print Functionality**: Use browser print to generate hard copies
...
1. **Access Settings**
   ...
   - Only administrators (or users with the correct permissions) can access this section

2. **Time & Work Rules**
   - Define standard working hours and break times
   - Configure overtime rules if your organization uses overtime
   - Align shift and attendance rules with company policies

3. **Holiday Management**
   - Add company holidays and update the holiday calendar
   - Ensure holidays are reflected correctly in attendance and payroll
...
1. **Create User Accounts**
   ...
   - Click **"Add User"**
   - Fill user details and assign the appropriate role

2. **Role Management**
   - Navigate to **"Users"** → **"Roles"**
   - Create or update roles (e.g. Admin, HR, Manager, Employee)
   - Assign specific permissions to each role based on responsibilities

3. **Permission Settings**
   - Define what each role can access (view, create, edit, delete)
   - Control access to sensitive areas like Payroll, Reports, and System Settings
...
1. **Subscription Status**
   - Some features may be restricted based on your subscription plan
   - If your subscription is close to expiring, a **warning modal** may appear in the application

2. **Expired Subscription**
   - When the subscription expires, users may be redirected to a **Subscription Expired** page
   - Contact your provider or system administrator to renew or upgrade your plan

3. **Access Restrictions**
   - If you see an **"Unauthorized"** page, your role does not have access to that section
   - Request the appropriate permissions from your administrator if required for your job
...
4. **Report Generation Problems**
   ...
   - **Solution**: Check date range and filters, clear browser cache, try a different browser
...
6. **Export Issues**
   - **Issue**: Cannot download Excel/PDF exports
   - **Solution**: Disable pop-up blockers for the site, ensure you have permission for reports, try again or contact support

7. **Subscription or Permission Errors**
   - **Issue**: See **"Unauthorized"** or **"Subscription Expired"** message
   - **Solution**: Contact your administrator to review your permissions or renew the subscription
