...
const PaymentPage = () => {
    ...
    const handleRegisterSubmit = async () => {
        try {
            setRegisterErrors({});
            await agreementSchema.validate(registerForm, { abortEarly: false });
            ...
            if (err.name === "ValidationError") {
                const fieldErrors = {};
                err.inner.forEach(error => { fieldErrors[error.path] = error.message; });
                ...
                ${billingCycle === cycle
                        ...
                        <div className="bg-white rounded-2xl border border-gray-300 shadow-sm p-8">
                            ...
                            <form className="space-y-4" onSubmit={handleSubmit}>
                                ...
                                <button type="submit"
                                    className="lg:hidden w-full py-3 rounded-xl bg-gradient-to-r from-[#6C4CF1] to-[#4B2EDB] text-white font-bold text-sm hover:opacity-90 active:scale-95 transition-all shadow-lg shadow-[#6C4CF1]/30">
                                    Pay Rs.{fmt(total)}
                                </button>
                            </form>
                        </div>
